...
namespace coflux {
    ...
    struct awaiter<detail::when_any_pair<TaskLikes...>, Executor> : public detail::maysuspend_awaiter_base {
    public:
    ...
    public:
        explicit awaiter(task_type&& basic_tasks, executor_pointer exec, std::atomic<status>* p)
            : basic_tasks_(std::move(basic_tasks))
    ...
    public:
        explicit awaiter(std::size_t n, Range&& basic_tasks, executor_pointer exec, std::atomic<status>* p)
            : n_(n)
            , basic_tasks_(std::forward<Range>(basic_tasks))
        ...
        bool await_suspend(std::coroutine_handle<Promise> handle) {
            ...
            auto callback = [
                handle, n = n_, result = result_, exec = executor_, 
                &error = error_, &stop = stop_source_, 
    ...
    auto when_any(TaskLikes&&...tasks) {
        return detail::when_any_pair<TaskLikes...>(detail::when_any_tag{}, std::tuple<TaskLikes...>(std::forward<TaskLikes>(tasks)...));
    }
